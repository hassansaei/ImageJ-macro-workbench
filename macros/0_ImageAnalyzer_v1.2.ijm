...
macro "Image Analyzer → DAPI and Multi-Channel Processing — ALL FILES (v1.2)" {
    // 1) Pick folder
    ...
    panelLargeOptions = newArray("Merged (DAPI)", "DAPI", "DAPI+C2", "DAPI+C3", "DAPI+C4", "No DAPI", "C2", "C3", "C4", "C2+C3", "C2+C4", "C3+C4", "C1+C2+C3", "C1+C2+C4", "C1+C3+C4", "C2+C3+C4");
    panelSmallOptions = newArray("None", "DAPI", "DAPI+C2", "DAPI+C3", "DAPI+C4", "No DAPI", "C2", "C3", "C4", "C2+C3", "C2+C4", "C3+C4", "C1+C2+C3", "C1+C2+C4", "C1+C3+C4", "C2+C3+C4");
    ...
    for (i=0; i<files.length; i++) {
        ...
        // Three-channel composites (all combinations of available channels, including DAPI if present)
...
function getTwoChannelMergeSpecSimple(channelIndex, dapiTitle, channelTitle, dapiColor, channelColor) {
    mergeSpec = "";
    mergeSpec = appendMergeSlot(mergeSpec, getColorChannelMapping(dapiColor), dapiTitle);
